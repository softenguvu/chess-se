...
QUnit.module("pawn", function() {
    ...
    QUnit.test("possibleMoves() after never having moved", function(assert) {
        ...
        assert.equal(movesReturn[0][0] == 5 && movesReturn[0][1] == 7, true, 'White piece at [7, 7], two spots ahead');
        assert.equal(movesReturn[1][0] == 6 && movesReturn[1][1] == 7, true, 'White piece at [7, 7], one spot ahead');

        testingPawn = new Pawn(1, 0, 0, 1); // Now try the piece as black
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[0][0] == 2 && movesReturn[0][1] == 0, true, 'Black piece at [0, 0], two spots ahead');
        assert.equal(movesReturn[1][0] == 1 && movesReturn[1][1] == 0, true, 'Black piece at [0, 0], one spot ahead');
    });
    ...
    QUnit.test("possibleMoves() if there is a piece on one side", function(assert) {
        var testingEnemy = new Pawn(3, 6, 6, 1); // Create a enemy pawn to current testing pawn
        var testingFriend = new Pawn(2, 6, 6, 0); // Create a friendly pawn to current testing pawn
        
        testingBoard.board[6][6] = testingEnemy; // Place enemy on board
        ...
        testingBoard.board[6][6] = testingFriend; // Replace enemy with friend
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        ...
        testingPawn = new Pawn(1, 0, 0, 1); // Now try as a black piece
        testingEnemy = new Pawn(2, 1, 1, 0);  // Recreate enemy and friend to match team switch of testing
        testingFriend = new Pawn(3, 1, 1, 1); 
        ...
        testingBoard.board[1][1] = testingFriend;
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn.length == 2, true, 'There should only be two possible moves, since a friend is now occupying the diagonal space');
    });

    QUnit.test("possibleMoves() if there is a piece on other side", function(assert) {
        testingPawn = new Pawn(1, 7, 5, 0); // Need to move the pawn so we can test the other side
        var testingEnemy = new Pawn(2, 6, 6, 1); // Create a enemy pawn to current testing pawn
        var testingFriend = new Pawn(3, 6, 6, 0); // Create a friendly pawn to current testing pawn
        
        testingBoard.board[6][6] = testingEnemy; // Place enemy on board
        var movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 6 && movesReturn[2][1] == 6, true, 'Enemy piece at [1, 1], which is diagonal to test piece');
        
        testingBoard.board[6][6] = testingFriend; // Replace enemy with friend
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn.length == 2, true, 'There should only be two possible moves, since a friend is now occupying the diagonal space');

        testingPawn = new Pawn(1, 0, 2, 1); // Now try as a black piece
        testingEnemy = new Pawn(3, 1, 1, 0); // Recreate enemy and friend to match team switch of testing
        testingFriend = new Pawn(2, 1, 1, 1);
        testingBoard.board[1][1] = testingEnemy;
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 1 && movesReturn[2][1] == 1, true, 'Enemy piece at [6, 6], which is diagonal to test piece');
    ...
    QUnit.test("possibleMoves() if there is a piece on both sides at once", function(assert) {
        ...
        var testingEnemy1 = new Pawn(2, 6, 6, 1); // Create a enemy pawn to current testing pawn
        var testingEnemy2 = new Pawn(4, 6, 4, 1); // Create a enemy pawn to current testing pawn
        var testingFriend1 = new Pawn(3, 6, 6, 0); // Create a friendly pawn to current testing pawn
        var testingFriend2 = new Pawn(5, 6, 4, 0); // Create a friendly pawn to current testing pawn

        // Place enemies on board
        testingBoard.board[6][6] = testingEnemy1; 
        testingBoard.board[6][4] = testingEnemy2;
        var movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 6 && movesReturn[2][1] == 4 && movesReturn[3][0] == 6 && movesReturn[3][1] == 6, true, 'Enemy pieces on both diagonal');
        
        // Replace one enemy with friend
        testingBoard.board[6][6] = testingFriend1; 
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 6 && movesReturn[2][1] == 4 && movesReturn.length == 3, true, 'Enemy pieces on one diagonal with a friend on the other');
        ...
        testingBoard.board[6][6] = testingEnemy1;
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 6 && movesReturn[2][1] == 6 && movesReturn.length == 3, true, 'Enemy pieces on one diagonal with a friend on the other, but swapped from previous test');
        ...
        testingPawn = new Pawn(1, 0, 2, 1); 
        var testingEnemy1 = new Pawn(2, 1, 1, 0); // Create a enemy pawn to current testing pawn
        var testingEnemy2 = new Pawn(4, 1, 3, 0); // Create a enemy pawn to current testing pawn
        var testingFriend1 = new Pawn(3, 1, 1, 1); // Create a friendly pawn to current testing pawn
        var testingFriend2 = new Pawn(5, 1, 3, 1); // Create a friendly pawn to current testing pawn
        
        // Place enemies on board
        testingBoard.board[1][1] = testingEnemy1;
        testingBoard.board[1][3] = testingEnemy2;
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 1 && movesReturn[2][1] == 3 && movesReturn[3][0] == 1 && movesReturn[3][1] == 1, true, 'Enemy pieces on both diagonals');
        
        // Now replace one enemy with a friend
        testingBoard.board[1][1] = testingFriend1;
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 1 && movesReturn[2][1] == 3 && movesReturn.length == 3, true, 'Enemy piece on one diagonal with a friend on the other');
        ...
        movesReturn = testingPawn.possibleMoves(testingBoard.board);
        assert.equal(movesReturn[2][0] == 1 && movesReturn[2][1] == 1 && movesReturn.length == 3, true, 'Enemy piece on one diagonal with a friend on the other, inverse from previous case');
    ...
    QUnit.test("possibleMoves() if there is a piece blocking forward movement", function(assert) {
        ...
        var testingEnemy = new Pawn(2, 6, 7, 1);
        var testingFriend = new Pawn(3, 6, 7, 0);
        ...
        var testingEnemy = new Pawn(2, 5, 7, 1)
        var testingFriend = new Pawn(3, 5, 7, 0);
        ...
        // Now try as a black piece
        testingPawn = new Pawn(1, 0, 0, 1)
        // Create more testing pieces
        var testingEnemy = new Pawn(2, 1, 0, 0);
        var testingFriend = new Pawn(3, 1, 0, 1);
        ...
        // Create testing pieces two squares in front
        var testingEnemy = new Pawn(2, 2, 0, 0)
        var testingFriend = new Pawn(3, 2, 0, 1);
    ...
    QUnit.test("possibleMoves() if there is a piece blocking forward movement, but there is a piece that is attackable", function(assert) {
        ...
        // Create more testing pieces
        var testingEnemy = new Pawn(2, 6, 6, 1);
        var testingFriend = new Pawn(3, 6, 7, 0);
        ...
        testingPawn = new Pawn(1, 0, 0, 1)
        // Create more testing pieces
        var testingEnemy = new Pawn(2, 1, 1, 0);
        var testingFriend = new Pawn(3, 1, 0, 1);
